...
class MarginWhiteSpaceScorer:
    ...
    """
    def __init__(self, slide: Slide, slide_width: Length, slide_height: Length, measurement_unit: str = "pt"):
        self._slide = slide
    ...
    def _get_bounding_box(self) -> dict:
        leftmost_x = 0
        rightmost_x = self._width
        topmost_y = 0
        bottommost_y = self._height
    ...
    def calculate_white_space_score(self) -> float:
        ...
        if margins['left'] > self._margin_threshold['horizontal'][0] and margins['left'] < self._margin_threshold['horizontal'][1]:
            horizontal_margin_score += 1
        if margins['right'] > self._margin_threshold['horizontal'][0] and margins['right'] < self._margin_threshold['horizontal'][1]:
            horizontal_margin_score += 1
        if margins['top'] > self._margin_threshold['vertical'][0] and margins['top'] < self._margin_threshold['vertical'][1]:
            vertical_margin_score += 1
        if margins['bottom'] > self._margin_threshold['vertical'][0] and margins['bottom'] < self._margin_threshold['vertical'][1]:
            vertical_margin_score += 1
